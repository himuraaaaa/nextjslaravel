...
class AdminReportController extends Controller
{
    ...
    public function exportResults(Request $request)
    {
        ...
        foreach ($attempts as $attempt) {
            $jumlahSoal = $attempt->testAnswers->count();
            $jumlahBenar = $attempt->testAnswers->where('is_correct', true)->count();
            $jumlahSalah = $attempt->testAnswers->where('is_correct', false)->count();
            $csvData[] = [
                $attempt->user->id,
                $attempt->user->name,
                $attempt->user->email,
