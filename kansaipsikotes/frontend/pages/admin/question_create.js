...
const QuestionCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Untuk essay, options dan correct_answer tidak dikirim
    try {
      await createQuestion(payload);
      router.push(`/admin/question_list?test_id=${test_id}`);
    } catch (err) {
      const errorMessage = err.response?.data?.message || err.message || 'Gagal membuat soal';
      ...
      <div className="max-w-4xl mx-auto py-6 px-4">
        ...
        <div className="bg-white shadow rounded-lg">
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {/* Question Text & Image */}
            <div>
              <label className="block text-sm font-medium mb-1 text-black">Pertanyaan *</label>
              <textarea
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Points */}
              <div>
                <label className="block text-sm font-medium mb-1 text-black">Poin *</label>
                <input
                  type="number"
                  className="w-full border border-gray-300 rounded px-3 py-2 mb-4 text-black"
                  value={points}
            ...
            {questionType === 'multiple_choice' && (
              <div className="space-y-4 pt-4 border-t">
                ...
                <p className="text-gray-700 text-black">Pilih salah satu opsi sebagai jawaban yang benar.</p>
            ...
            <div className="flex justify-end pt-6 border-t">
              <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Menyimpan...' : 'Simpan Soal'}</button>
            </div>
          </form>
        </div>
      </div>
