...
const TestResults = () => {
  ...
  const [detailsMap, setDetailsMap] = useState({}); // { [resultId]: {totalQuestions, totalCorrect, totalWrong} }
  ...
  // Fetch detail per result untuk summary soal/benar/salah
  useEffect(() => {
    const fetchDetails = async () => {
      const idsToFetch = paginatedResults
        .filter(r => !detailsMap[r.id])
        .map(r => r.id);
      ...
      await Promise.all(idsToFetch.map(async (id) => {
        try {
          ...
          const answers = res.data.test_answers || [];
          newDetails[id] = {
            totalQuestions: answers.length,
            totalCorrect: answers.filter(a => a.is_correct).length,
            totalWrong: answers.filter(a => !a.is_correct).length,
          };
        } catch (e) {
          newDetails[id] = { totalQuestions: '-', totalCorrect: '-', totalWrong: '-' };
        }
      }));
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Jumlah Soal</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Jumlah Benar</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Jumlah Salah</th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {paginatedResults.map((result) => {
                  ...
                  const totalQuestions = detail ? detail.totalQuestions : null;
                  const totalCorrect = detail ? detail.totalCorrect : null;
                  const totalWrong = detail ? detail.totalWrong : null;
                  return (
                    <tr key={result.id} className="hover:bg-blue-50 transition-all">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{totalQuestions === null ? <span className="text-gray-400">...</span> : totalQuestions}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-green-700 font-bold">{totalCorrect === null ? <span className="text-gray-400">...</span> : totalCorrect}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-red-700 font-bold">{totalWrong === null ? <span className="text-gray-400">...</span> : totalWrong}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button
                          onClick={() => window.location.href = `/admin/test-results/${result.id}`}
      ...
      {showDetail && selectedResult && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-8 relative animate-fade-in">
            ...
            <h2 className="text-2xl font-bold mb-4 text-blue-900">Detail Hasil Test</h2>
            <div className="mb-4">
              <div className="font-semibold text-gray-700">Nama: <span className="font-bold text-gray-900">{selectedResult.user?.name || '-'}</span></div>
              <div className="font-semibold text-gray-700">Email: <span className="text-gray-900">{selectedResult.user?.email || '-'}</span></div>
              <div className="font-semibold text-gray-700">Test: <span className="text-gray-900">{selectedResult.test?.title || '-'}</span></div>
              <div className="font-semibold text-gray-700">Status: <span className="text-gray-900">{selectedResult.status || '-'}</span></div>
              <div className="font-semibold text-gray-700">Skor: <span className="text-gray-900">{selectedResult.score ?? '-'}</span></div>
              <div className="font-semibold text-gray-700">Durasi: <span className="text-gray-900">{getDuration(selectedResult.started_at, selectedResult.completed_at)}</span></div>
              <div className="font-semibold text-gray-700">Tanggal: <span className="text-gray-900">{formatDate(selectedResult.completed_at)}</span></div>
            </div>
            {/* Tampilkan detail soal/jawaban jika ada */}
            {selectedResult.answers && (
              <div className="mt-6">
                <h3 className="text-lg font-bold mb-2 text-blue-800">Jawaban Peserta</h3>
                <div className="overflow-x-auto">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {selectedResult.answers.map((ans, idx) => (
                        <tr key={ans.id || idx}>
                          ...
                          <td className="px-3 py-2">{ans.question_text || '-'}</td>
                          <td className="px-3 py-2">{ans.answer_text || (typeof ans.answer === 'string' ? ans.answer : ans.answer ? JSON.stringify(ans.answer) : '-')}</td>
                          <td className="px-3 py-2">{ans.is_correct ? <span className="text-green-600 font-bold">Ya</span> : <span className="text-red-600 font-bold">Tidak</span>}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
