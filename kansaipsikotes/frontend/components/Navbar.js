...
const Navbar = () => {
  ...
  const [notif, setNotif] = useState(null); // {userId, text}
  ...
  return (
    <nav className="shadow-md border-b relative z-20" style={{ background: '#001F5A' }}>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {isAdmin && (
              <>
                ...
                <Link href="/admin/review" className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors ${router.pathname === '/admin/review' ? 'bg-green-100 text-green-700' : 'text-white hover:text-green-200 hover:bg-white hover:bg-opacity-10'}`}><BarChart3 className="h-4 w-4" /><span>Review Soal</span></Link>
                <Link href="/admin/live-monitor" className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors ${router.pathname === '/admin/live-monitor' ? 'bg-green-100 text-green-700' : 'text-white hover:text-green-200 hover:bg-white hover:bg-opacity-10'}`}><BarChart3 className="h-4 w-4" /><span>Live Monitoring</span></Link>
              </>
            )}
          </div>
      ...
      {mobileMenuOpen && (
        ...
        <div className="px-2 pt-2 pb-3 space-y-1">
            ...
            <>
                ...
                <Link href="/admin/review" className={`block px-3 py-2 rounded-md text-base font-medium ${router.pathname === '/admin/review' ? 'bg-green-100 text-green-700' : 'text-white hover:text-green-200 hover:bg-white hover:bg-opacity-10'}`} onClick={() => setMobileMenuOpen(false)}>Review Soal</Link>
                <Link href="/admin/live-monitor" className={`block px-3 py-2 rounded-md text-base font-medium ${router.pathname === '/admin/live-monitor' ? 'bg-green-100 text-green-700' : 'text-white hover:text-green-200 hover:bg-white hover:bg-opacity-10'}`} onClick={() => setMobileMenuOpen(false)}>Live Monitoring</Link>
                <Link href="/admin/chat" className={`block px-3 py-2 rounded-md text-base font-medium ${router.pathname === '/admin/chat' ? 'bg-yellow-100 text-yellow-700' : 'text-white hover:text-yellow-600 hover:bg-white hover:bg-opacity-10'}`} onClick={() => setMobileMenuOpen(false)}>Chat</Link>
              </>
            )}
      ...
      {/* Notifikasi float global */}
      {notif && (
        <div
          className="fixed bottom-8 right-8 z-50 bg-yellow-400 text-gray-900 px-4 py-3 rounded-lg shadow-lg font-bold cursor-pointer animate-bounce flex items-center gap-2"
          onClick={() => {
            setNotif(null);
            router.push(`/admin/chat?user=${notif.userId}`);
          }}
        >
          <span className="inline-block bg-white text-yellow-600 rounded-full px-2 py-0.5 mr-2 font-bold text-xs">Pesan Baru</span>
          <span className="font-semibold mr-2">{notif.userId}</span>
          <span className="truncate max-w-[120px]">{notif.text}</span>
        </div>
      )}
    </nav>
  );
